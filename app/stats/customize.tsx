...
import { parseStatsLayout, stringifyStatsLayout, STATS_LABELS, type StatsItem } from '@/lib/stats_layout';
import { LONG_PRESS_MS } from '@/lib/gestures';
import { BOTTOM_BAR_PADDING } from '@/lib/layout';
import * as haptic from '@/lib/haptic';

export default function CustomizeStats() {
  const palette = useThemePalette();
  const router = useRouter();
  const layoutJson = useAppStore((s) => s.statsLayoutJson);
  const setLayoutJson = useAppStore((s) => s.setStatsLayoutJson);
  const [items, setItems] = useState<StatsItem[]>([]);

  useEffect(() => {
    setItems(parseStatsLayout(layoutJson).items);
  }, [layoutJson]);

  const onSave = async () => {
    haptic.success();
    const ordered = items.map((c, i) => ({ ...c, order: i }));
    await setLayoutJson(stringifyStatsLayout({ items: ordered }));
    router.back();
  };

  const renderItem = ({ item, drag, isActive }: RenderItemParams<StatsItem>) => (
    <Pressable
    ...
    >
      ...
      <Text style={{ color: palette.text, fontWeight: '600', flex: 1 }}>{STATS_LABELS[item.id]}</Text>
      <Switch
        value={item.visible}
        onValueChange={(v) => {
          haptic.tapLight();
          setItems((prev) => prev.map((c) => c.id === item.id ? { ...c, visible: v } : c));
        }}
      />
    </Pressable>
  );
