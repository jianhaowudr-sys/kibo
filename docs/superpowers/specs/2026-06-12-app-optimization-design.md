...
| 量測點 | 改前 | 改後 |
|---|---|---|
| ensureSchema | 待使用者實機量測 | 待使用者實機量測 |
| bootstrap | 待使用者實機量測 | 待使用者實機量測 |
| total-to-ready | 待使用者實機量測 | 待使用者實機量測 |
| AI 首結果 | 待使用者實機量測 | 待使用者實機量測 |
| AI 複核完成 | 待使用者實機量測 | 待使用者實機量測 |

## 驗收狀態（2026-06-12 實作完成）

**已自動驗證（本機）：**
- `npx tsc --noEmit` 全綠（每個 task 完成時皆通過）。
- `npx -y tsx scripts/verify_meal_logic.ts` → ALL PASS (12 checks)：複核差異比對 / 快照欄位保護 / 摘要格式化純函數。

**待使用者在實機驗收（無裝置／API Key 無法於開發環境完成）：**
- 冷啟動 `[perf]` 改前／改後數字對比（填入上方附錄表）。
- AI 兩段式 UI 三態流轉、低耗單次、失敗 fallback（需 API Key）。
- AI 準度實拍 3~5 張（便當、麵食、手搖飲）對照舊版，檢查漏項與份量。
- 回歸：onboarding 跳轉、主題無閃爍、營養標／InBody 不受影響、多照片流程、使用者改過欄位不被複核覆蓋。

**實作期間發現、本輪未處理（已記於計畫附錄「超出上限的發現」並另開背景任務）：**
- `app/diet/new.tsx` save() 單筆分支硬編 `loggedAt: new Date()`，補登過去日期會記成「現在」（既有問題，非本輪回歸）。
- 多照片「不同餐分別記」遇單張判讀失敗時，因 `perPhotoReadings.length !== photos.length` 靜默退回單筆合併儲存（既有行為）。
